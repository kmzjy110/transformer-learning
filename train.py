...
def model_train(num_epochs, batch_size, lr, model_file):
    ...
    dataset = icl_dataset.ICLDataset()
    train_length = int(len(dataset) * 0.9)
    valid_length = len(dataset) - train_length
    train_set, valid_set = torch.utils.data.random_split(dataset, [train_length,valid_length])

    train_dataloader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=True, collate_fn=icl_dataset.ICLDataset.collate)
    valid_dataloader = torch.utils.data.DataLoader(valid_set, batch_size=batch_size, shuffle=True, collate_fn=icl_dataset.ICLDataset.collate)
    optimizer = torch.optim.Adam(model.parameters(), betas=(0.9,0.999), eps = 1e-9)
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, lr, epochs=num_epochs, steps_per_epoch = len(train_dataloader), pct_start=0.1)
    train_losses = []
